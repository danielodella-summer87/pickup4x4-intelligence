...
import { replaceItems } from "@/lib/campanas/campaign-server";
import type { CampaignItemDTO } from "@/lib/campanas/campaign-types";
...
export async function POST(request: Request, { params }: Context) {
  ...
  let body: { items?: CampaignItemDTO[] };
  try {
    body = (await request.json()) as { items?: CampaignItemDTO[] };
  } catch {
    return NextResponse.json({ ok: false, errorMessage: "JSON inválido." }, { status: 400 });
  }

  const result = await replaceItems(id, body.items ?? []);
  if (!result.ok) {
